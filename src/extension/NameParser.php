...
class NameParser {
    /*
     * Adapted to Lettuce Class by Ben Blazely 2016
     *
    Name:	nameparse.php
    Version: 0.2a
    Date:	030507
        ...
        switch ($n_pieces) {
            case 1:    // array(title first middles last suffix)
                ...
                for ($i = 0; $i < $n_subp; $i++) {
                    ...
                    if ($i == 0 && $this->in_array_norm($curr, $titles)) {
                        $out['title'] = $curr;
                        continue;
                    }
                    ...
                    if ($i == $n_subp - 2 && $next && $this->in_array_norm($next, $suffices)) {
                        if ($out['last']) {
                            $out['last'] .= " $curr";
                        } else {
                            $out['last'] = $curr;
                    ...
                    if ($this->in_array_norm($curr, $prefices)) {
                        if (isset($out['last'])) {
                            $out['last'] .= " $curr";
                        } else {
                            $out['last'] = $curr;
                ...
                break;
            case 2:
                if ($this->in_array_norm($pieces[1], $suffices)) {
                    // array(title first middles last,suffix)
                    $subp = explode(' ', trim($pieces[0]));
                    $n_subp = count($subp);
                    for ($i = 0; $i < $n_subp; $i++) {
                        $curr = trim($subp[$i]);
                    ...
                    // array(last,title first middles suffix)
                    ...
                    for ($i = 0; $i < $n_subp; $i++) {
                        ...
                        if ($i == $n_subp - 2 && $next &&
                            $this->in_array_norm($next, $suffices)
                        ) {
                            if ($out['middle']) {
                                $out['middle'] .= " $curr";
                            } else {
                        ...
                        if ($i == $n_subp - 1 && $this->in_array_norm($curr, $suffices)) {
                            $out['suffix'] = $curr;
                            continue;
                        }
